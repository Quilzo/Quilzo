...
// markGenerated marks every draft page as model-made, so a test can be about
// something other than the provenance gate.
//
// A local helper here and a command on main, where `import` and the assistant
// both call it. Adding it as production code on this branch would put a
// function in the tree that nothing runs, which is the shape of defect half
// these backported fixes are about.
