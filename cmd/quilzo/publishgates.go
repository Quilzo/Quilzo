...
func contentGates(root string, s *store.Store, ref string) gate.Set {
	...
	return gate.Set{
		...
		// being published, so it is asked here and not when somebody saves.
		{
			Name: "references",
			Refusal: func(n int) string {
				return fmt.Sprintf("%d page(s) reference a page that is not "+
					"being published.\n  Publish the page it names, or change "+
					"the reference", n)
			},
			Run: func() (blocking, advisory []gate.Finding, err error) {
				pages, perr := pagesAt()
				...
				for _, f := range st.Unresolved(pages) {
					blocking = append(blocking, gate.Finding{Detail: f.String()})
				}
