...
questionnaires
  ...
  quilzo attest fill FILE [-o OUT]         answer one, checked against the register
...
working together
  ...
  quilzo checked own PAGE WHO | disown    whose job a page is
...
func main() {
	...
	case "links":
		err = cmdLinks(root, cmdArgs)
	case "move":
		err = cmdMove(root, cmdArgs)
	case "diff":
		err = cmdDiff(root)
	case "publish":
